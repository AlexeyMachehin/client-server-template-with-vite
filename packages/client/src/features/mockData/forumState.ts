import { IForumState } from '../../service/types/forumPage/IForumState';

export const forumState: IForumState = {
    discussionOfGameMoments: [
      {
        id: 100500,
        name: 'Danil',
        title: 'discussionOfGameMoment 1M',
        time: '2020-01-02T14:22:22.000Z',
        content: 'this is message content',
        avatarURL:
          'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
        messages: [
          {
            id: 1,
            isMyMessage: true,
            name: 'Danilth',
            time: '2020-01-02T14:22:22.000Z',
            message:
              '',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 2,
            isMyMessage: false,
            name: 'Daniltrhb5',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'wwwwwwwww wwwwwwWWWWWWWW WWWWWWWWWWWWWWWWW WWWWWWWWWWWWW WWWWWWWWWWWWWWWWW WWWWAwwwwwwwww wwwwwwWWWWWWW WWWWWWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWA wwwww wwwwwwwwwWWWWWWWW WWWWWWWWWWWWW WWWWWWWWWWAwwwwwww wwwwwwwwWWW WWWWWWWWWWWWWW WWWWWWWWWWWWWW WWWWWWWWWWWWWWW WWWWWWWWWWWWWW Awwwwwwwwwwww wwwWWWWWWWWWW WWWWWWWWWWWWWW WWWWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWW WAwwwwwwwww wwwwwwWWWWWWWWWWW WWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWA',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 3,
            isMyMessage: true,
            name: 'Danildfg345t',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'тексregergт сообщкст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщения',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },

          {
            id: 6,
            isMyMessage: true,
            name: 'Da54gnil',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'текст сообщения trhrtтекст соониятекст сообщениятекст сообщениятекст сообщениятекст сообщения',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 7,
            isMyMessage: true,
            name: 'D54anil',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'текст сообщения текст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщения',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 8,
            isMyMessage: true,
            name: 'Da5g4nil',
            time: '2020-01-02T14:22:22.000Z',
            message: 'текст сообщения текст сообщениятятекст сообщениятекст',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
        ],
      },
      {
        id: 9,
        name: 'Danil',
        title: 'discussionOfGameMoment 2F',
        time: '2020-01-02T14:22:22.000Z',
        content: 'this is message content',
        avatarURL:
          'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
        messages: [
          {
            id: 10,
            isMyMessage: true,
            name: 'Danilth',
            time: '2020-01-02T14:22:22.000Z',
            message:
              '',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 11,
            isMyMessage: false,
            name: 'Daniltrhb5',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'wwwwwwwww wwwwwwWWWWWWWW WWWWWWWWWWWWWWWWW WWWWWWWWWWWWW WWWWWWWWWWWWWWWWW WWWWAwwwwwwwww wwwwwwWWWWWWW WWWWWWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWA wwwww wwwwwwwwwWWWWWWWW WWWWWWWWWWWWW WWWWWWWWWWAwwwwwww wwwwwwwwWWW WWWWWWWWWWWWWW WWWWWWWWWWWWWW WWWWWWWWWWWWWWW WWWWWWWWWWWWWW Awwwwwwwwwwww wwwWWWWWWWWWW WWWWWWWWWWWWWW WWWWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWW WAwwwwwwwww wwwwwwWWWWWWWWWWW WWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWA',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 12,
            isMyMessage: true,
            name: 'Danildfg345t',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'тексregergт сообщкст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщения',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
        ],
      },
    ],

    technicalIssues: [
      {
        id: 13,
        name: 'Danil',
        title: 'technicalIssue 1',
        time: '2020-01-02T14:22:22.000Z',
        content: 'this is message content',
        avatarURL:
          'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
        messages: [
          {
            id: 14,
            isMyMessage: true,
            name: 'Danilth',
            time: '2020-01-02T14:22:22.000Z',
            message:
              '',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 15,
            isMyMessage: false,
            name: 'Daniltrhb5',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'wwwwwwwww wwwwwwWWWWWWWW WWWWWWWWWWWWWWWWW WWWWWWWWWWWWW WWWWWWWWWWWWWWWWW WWWWAwwwwwwwww wwwwwwWWWWWWW WWWWWWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWA wwwww wwwwwwwwwWWWWWWWW WWWWWWWWWWWWW WWWWWWWWWWAwwwwwww wwwwwwwwWWW WWWWWWWWWWWWWW WWWWWWWWWWWWWW WWWWWWWWWWWWWWW WWWWWWWWWWWWWW Awwwwwwwwwwww wwwWWWWWWWWWW WWWWWWWWWWWWWW WWWWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWW WAwwwwwwwww wwwwwwWWWWWWWWWWW WWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWA',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 16,
            isMyMessage: true,
            name: 'Danildfg345t',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'тексregergт сообщкст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщения',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 17,
            isMyMessage: true,
            name: 'Dreg45anil',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'текст сообщения текст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщения',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 18,
            isMyMessage: false,
            name: 'D54ganil',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'текст сосообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщения',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 19,
            isMyMessage: true,
            name: 'Da54gnil',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'текст сообщения trhrtтекст соониятекст сообщениятекст сообщениятекст сообщениятекст сообщения',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 20,
            isMyMessage: true,
            name: 'D54anil',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'текст сообщения текст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщения',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 21,
            isMyMessage: true,
            name: 'Da5g4nil',
            time: '2020-01-02T14:22:22.000Z',
            message: 'текст сообщения текст сообщениятятекст сообщениятекст',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 22,
            isMyMessage: false,
            name: 'Da45gnil',
            time: '2020-01-02T14:22:22.000Z',
            message: 'текст',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
        ],
      },
      {
        id: 23,
        name: 'Danil',
        title: 'technicalIssue 2',
        time: '2020-01-02T14:22:22.000Z',
        content: 'this is message content',
        avatarURL:
          'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
        messages: [],
      },
    ],

    errorQuestions: [
      {
        id: 24,
        name: 'Danil',
        title:
          '',
        time: '2020-01-02T14:22:22.000Z',
        content: 'this is message content',
        avatarURL:
          'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
        messages: [
          {
            id: 25,
            isMyMessage: true,
            name: 'Danilth',
            time: '2020-01-02T14:22:22.000Z',
            message:
              '',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 26,
            isMyMessage: false,
            name: 'Daniltrhb5',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'wwwwwwwww wwwwwwWWWWWWWW WWWWWWWWWWWWWWWWW WWWWWWWWWWWWW WWWWWWWWWWWWWWWWW WWWWAwwwwwwwww wwwwwwWWWWWWW WWWWWWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWA wwwww wwwwwwwwwWWWWWWWW WWWWWWWWWWWWW WWWWWWWWWWAwwwwwww wwwwwwwwWWW WWWWWWWWWWWWWW WWWWWWWWWWWWWW WWWWWWWWWWWWWWW WWWWWWWWWWWWWW Awwwwwwwwwwww wwwWWWWWWWWWW WWWWWWWWWWWWWW WWWWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWW WAwwwwwwwww wwwwwwWWWWWWWWWWW WWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWA',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 27,
            isMyMessage: true,
            name: 'Danildfg345t',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'тексregergт сообщкст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщения',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 28,
            isMyMessage: true,
            name: 'Dreg45anil',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'текст сообщения текст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщения',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 29,
            isMyMessage: false,
            name: 'D54ganil',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'текст сосообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщения',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 30,
            isMyMessage: true,
            name: 'Da54gnil',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'текст сообщения trhrtтекст соониятекст сообщениятекст сообщениятекст сообщениятекст сообщения',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 7,
            isMyMessage: true,
            name: 'D54anil',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'текст сообщения текст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщения',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 8,
            isMyMessage: true,
            name: 'Da5g4nil',
            time: '2020-01-02T14:22:22.000Z',
            message: 'текст сообщения текст сообщениятятекст сообщениятекст',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 9,
            isMyMessage: false,
            name: 'Da45gnil',
            time: '2020-01-02T14:22:22.000Z',
            message: 'текст',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
        ],
      },
      {
        id: 2,
        name: 'Boris',
        title:
          'Error 500Error 500Error500 Error500 Error 500Error500Er ror500Error5 00Error500Err or500Erro r500Error50 0Error500Err or500Error500Error500Error5 00Error500Error500Error500Error 500Error500Error5 00Error500Error50 0Error500Error500Error500Error500Error500Error500Error500Error500Error500Error500Error500',
        time: '2020-01-02T14:22:22.000Z',
        content: 'this is message content',
        avatarURL:
          'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
        messages: [
          {
            id: 1,
            isMyMessage: true,
            name: 'Danilth',
            time: '2020-01-02T14:22:22.000Z',
            message:
              '',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
        ],
      },
      {
        id: 3,
        name: 'Anton',
        title: 'cError 300',
        time: '2020-01-02T14:22:22.000Z',
        content: 'this is message content',
        avatarURL:
          'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
        messages: [
          {
            id: 1,
            isMyMessage: true,
            name: 'Danilth',
            time: '2020-01-02T14:22:22.000Z',
            message:
              '',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 2,
            isMyMessage: false,
            name: 'Daniltrhb5',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'wwwwwwwww wwwwwwWWWWWWWW WWWWWWWWWWWWWWWWW WWWWWWWWWWWWW WWWWWWWWWWWWWWWWW WWWWAwwwwwwwww wwwwwwWWWWWWW WWWWWWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWA wwwww wwwwwwwwwWWWWWWWW WWWWWWWWWWWWW WWWWWWWWWWAwwwwwww wwwwwwwwWWW WWWWWWWWWWWWWW WWWWWWWWWWWWWW WWWWWWWWWWWWWWW WWWWWWWWWWWWWW Awwwwwwwwwwww wwwWWWWWWWWWW WWWWWWWWWWWWWW WWWWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWW WAwwwwwwwww wwwwwwWWWWWWWWWWW WWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWA',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
        ],
      },
      {
        id: 4,
        name: 'Katya',
        title: 'aError 200',
        time: '2020-01-02T14:22:22.000Z',
        content: 'this is message content',
        avatarURL:
          'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
        messages: [
          {
            id: 1,
            isMyMessage: true,
            name: 'Danilth',
            time: '2020-01-02T14:22:22.000Z',
            message:
              '',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 2,
            isMyMessage: false,
            name: 'Daniltrhb5',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'wwwwwwwww wwwwwwWWWWWWWW WWWWWWWWWWWWWWWWW WWWWWWWWWWWWW WWWWWWWWWWWWWWWWW WWWWAwwwwwwwww wwwwwwWWWWWWW WWWWWWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWA wwwww wwwwwwwwwWWWWWWWW WWWWWWWWWWWWW WWWWWWWWWWAwwwwwww wwwwwwwwWWW WWWWWWWWWWWWWW WWWWWWWWWWWWWW WWWWWWWWWWWWWWW WWWWWWWWWWWWWW Awwwwwwwwwwww wwwWWWWWWWWWW WWWWWWWWWWWWWW WWWWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWW WAwwwwwwwww wwwwwwWWWWWWWWWWW WWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWA',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 3,
            isMyMessage: true,
            name: 'Danildfg345t',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'тексregergт сообщкст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщения',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 4,
            isMyMessage: true,
            name: 'Dreg45anil',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'текст сообщения текст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщения',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 5,
            isMyMessage: false,
            name: 'D54ganil',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'текст сосообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщения',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 6,
            isMyMessage: true,
            name: 'Da54gnil',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'текст сообщения trhrtтекст соониятекст сообщениятекст сообщениятекст сообщениятекст сообщения',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 7,
            isMyMessage: true,
            name: 'D54anil',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'текст сообщения текст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщения',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 8,
            isMyMessage: true,
            name: 'Da5g4nil',
            time: '2020-01-02T14:22:22.000Z',
            message: 'текст сообщения текст сообщениятятекст сообщениятекст',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 9,
            isMyMessage: false,
            name: 'Da45gnil',
            time: '2020-01-02T14:22:22.000Z',
            message: 'текст',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
        ],
      },
      {
        id: 5,
        name: 'Masha',
        title: 'Error 100',
        time: '2020-01-02T14:22:22.000Z',
        content: 'this is message content',
        avatarURL:
          'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
        messages: [
          {
            id: 1,
            isMyMessage: true,
            name: 'Danilth',
            time: '2020-01-02T14:22:22.000Z',
            message:
              '',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 2,
            isMyMessage: false,
            name: 'Daniltrhb5',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'wwwwwwwww wwwwwwWWWWWWWW WWWWWWWWWWWWWWWWW WWWWWWWWWWWWW WWWWWWWWWWWWWWWWW WWWWAwwwwwwwww wwwwwwWWWWWWW WWWWWWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWA wwwww wwwwwwwwwWWWWWWWW WWWWWWWWWWWWW WWWWWWWWWWAwwwwwww wwwwwwwwWWW WWWWWWWWWWWWWW WWWWWWWWWWWWWW WWWWWWWWWWWWWWW WWWWWWWWWWWWWW Awwwwwwwwwwww wwwWWWWWWWWWW WWWWWWWWWWWWWW WWWWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWW WAwwwwwwwww wwwwwwWWWWWWWWWWW WWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWA',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          // {
          //   id: 3,
          //   isMyMessage: true,
          //   name: 'Danildfg345t',
          //   time: '2020-01-02T14:22:22.000Z',
          //   message:
          //     'тексregergт сообщкст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщения',
          //   avatarURL:
          //     'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          // },
          // {
          //   id: 4,
          //   isMyMessage: true,
          //   name: 'Dreg45anil',
          //   time: '2020-01-02T14:22:22.000Z',
          //   message:
          //     'текст сообщения текст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщения',
          //   avatarURL:
          //     'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          // },
          // {
          //   id: 5,
          //   isMyMessage: false,
          //   name: 'D54ganil',
          //   time: '2020-01-02T14:22:22.000Z',
          //   message:
          //     'текст сосообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщения',
          //   avatarURL:
          //     'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          // },
          // {
          //   id: 6,
          //   isMyMessage: true,
          //   name: 'Da54gnil',
          //   time: '2020-01-02T14:22:22.000Z',
          //   message:
          //     'текст сообщения trhrtтекст соониятекст сообщениятекст сообщениятекст сообщениятекст сообщения',
          //   avatarURL:
          //     'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          // },
          // {
          //   id: 7,
          //   isMyMessage: true,
          //   name: 'D54anil',
          //   time: '2020-01-02T14:22:22.000Z',
          //   message:
          //     'текст сообщения текст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщения',
          //   avatarURL:
          //     'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          // },
          // {
          //   id: 8,
          //   isMyMessage: true,
          //   name: 'Da5g4nil',
          //   time: '2020-01-02T14:22:22.000Z',
          //   message: 'текст сообщения текст сообщениятятекст сообщениятекст',
          //   avatarURL:
          //     'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          // },
          // {
          //   id: 9,
          //   isMyMessage: false,
          //   name: 'Da45gnil',
          //   time: '2020-01-02T14:22:22.000Z',
          //   message: 'текст',
          //   avatarURL:
          //     'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          // },
        ],
      },
      {
        id: 6,
        name: 'Danil',
        title: 'Error 500',
        time: '2020-01-02T14:22:22.000Z',
        content: 'this is message content',
        avatarURL:
          'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
        messages: [
          {
            id: 1,
            isMyMessage: true,
            name: 'Danilth',
            time: '2020-01-02T14:22:22.000Z',
            message:
              '',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 2,
            isMyMessage: false,
            name: 'Daniltrhb5',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'wwwwwwwww wwwwwwWWWWWWWW WWWWWWWWWWWWWWWWW WWWWWWWWWWWWW WWWWWWWWWWWWWWWWW WWWWAwwwwwwwww wwwwwwWWWWWWW WWWWWWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWA wwwww wwwwwwwwwWWWWWWWW WWWWWWWWWWWWW WWWWWWWWWWAwwwwwww wwwwwwwwWWW WWWWWWWWWWWWWW WWWWWWWWWWWWWW WWWWWWWWWWWWWWW WWWWWWWWWWWWWW Awwwwwwwwwwww wwwWWWWWWWWWW WWWWWWWWWWWWWW WWWWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWW WAwwwwwwwww wwwwwwWWWWWWWWWWW WWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWA',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 3,
            isMyMessage: true,
            name: 'Danildfg345t',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'тексregergт сообщкст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщения',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 4,
            isMyMessage: true,
            name: 'Dreg45anil',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'текст сообщения текст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщения',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 5,
            isMyMessage: false,
            name: 'D54ganil',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'текст сосообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщения',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 6,
            isMyMessage: true,
            name: 'Da54gnil',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'текст сообщения trhrtтекст соониятекст сообщениятекст сообщениятекст сообщениятекст сообщения',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 7,
            isMyMessage: true,
            name: 'D54anil',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'текст сообщения текст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщения',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 8,
            isMyMessage: true,
            name: 'Da5g4nil',
            time: '2020-01-02T14:22:22.000Z',
            message: 'текст сообщения текст сообщениятятекст сообщениятекст',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 9,
            isMyMessage: false,
            name: 'Da45gnil',
            time: '2020-01-02T14:22:22.000Z',
            message: 'текст',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
        ],
      },
      {
        id: 7,
        name: 'Boris',
        title: 'Error 400',
        time: '2020-01-02T14:22:22.000Z',
        content: 'this is message content',
        avatarURL:
          'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
        messages: [
          {
            id: 1,
            isMyMessage: true,
            name: 'Danilth',
            time: '2020-01-02T14:22:22.000Z',
            message:
              '',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 2,
            isMyMessage: false,
            name: 'Daniltrhb5',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'wwwwwwwww wwwwwwWWWWWWWW WWWWWWWWWWWWWWWWW WWWWWWWWWWWWW WWWWWWWWWWWWWWWWW WWWWAwwwwwwwww wwwwwwWWWWWWW WWWWWWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWA wwwww wwwwwwwwwWWWWWWWW WWWWWWWWWWWWW WWWWWWWWWWAwwwwwww wwwwwwwwWWW WWWWWWWWWWWWWW WWWWWWWWWWWWWW WWWWWWWWWWWWWWW WWWWWWWWWWWWWW Awwwwwwwwwwww wwwWWWWWWWWWW WWWWWWWWWWWWWW WWWWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWW WAwwwwwwwww wwwwwwWWWWWWWWWWW WWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWA',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 3,
            isMyMessage: true,
            name: 'Danildfg345t',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'тексregergт сообщкст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщения',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 4,
            isMyMessage: true,
            name: 'Dreg45anil',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'текст сообщения текст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщения',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 5,
            isMyMessage: false,
            name: 'D54ganil',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'текст сосообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщения',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 6,
            isMyMessage: true,
            name: 'Da54gnil',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'текст сообщения trhrtтекст соониятекст сообщениятекст сообщениятекст сообщениятекст сообщения',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 7,
            isMyMessage: true,
            name: 'D54anil',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'текст сообщения текст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщения',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 8,
            isMyMessage: true,
            name: 'Da5g4nil',
            time: '2020-01-02T14:22:22.000Z',
            message: 'текст сообщения текст сообщениятятекст сообщениятекст',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 9,
            isMyMessage: false,
            name: 'Da45gnil',
            time: '2020-01-02T14:22:22.000Z',
            message: 'текст',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
        ],
      },
      {
        id: 8,
        name: 'Anton',
        title: 'dError 300',
        time: '2020-01-02T14:22:22.000Z',
        content: 'this is message content',
        avatarURL:
          'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
        messages: [
          {
            id: 1,
            isMyMessage: true,
            name: 'Danilth',
            time: '2020-01-02T14:22:22.000Z',
            message:
              '',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 2,
            isMyMessage: false,
            name: 'Daniltrhb5',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'wwwwwwwww wwwwwwWWWWWWWW WWWWWWWWWWWWWWWWW WWWWWWWWWWWWW WWWWWWWWWWWWWWWWW WWWWAwwwwwwwww wwwwwwWWWWWWW WWWWWWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWA wwwww wwwwwwwwwWWWWWWWW WWWWWWWWWWWWW WWWWWWWWWWAwwwwwww wwwwwwwwWWW WWWWWWWWWWWWWW WWWWWWWWWWWWWW WWWWWWWWWWWWWWW WWWWWWWWWWWWWW Awwwwwwwwwwww wwwWWWWWWWWWW WWWWWWWWWWWWWW WWWWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWW WAwwwwwwwww wwwwwwWWWWWWWWWWW WWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWA',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 3,
            isMyMessage: true,
            name: 'Danildfg345t',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'тексregergт сообщкст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщения',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 4,
            isMyMessage: true,
            name: 'Dreg45anil',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'текст сообщения текст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщения',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 5,
            isMyMessage: false,
            name: 'D54ganil',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'текст сосообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщения',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 6,
            isMyMessage: true,
            name: 'Da54gnil',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'текст сообщения trhrtтекст соониятекст сообщениятекст сообщениятекст сообщениятекст сообщения',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 7,
            isMyMessage: true,
            name: 'D54anil',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'текст сообщения текст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщения',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 8,
            isMyMessage: true,
            name: 'Da5g4nil',
            time: '2020-01-02T14:22:22.000Z',
            message: 'текст сообщения текст сообщениятятекст сообщениятекст',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 9,
            isMyMessage: false,
            name: 'Da45gnil',
            time: '2020-01-02T14:22:22.000Z',
            message: 'текст',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
        ],
      },
      {
        id: 9,
        name: 'Katya',
        title: 'Error 200',
        time: '2020-01-02T14:22:22.000Z',
        content: 'this is message content',
        avatarURL:
          'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
        messages: [
          {
            id: 1,
            isMyMessage: true,
            name: 'Danilth',
            time: '2020-01-02T14:22:22.000Z',
            message:
              '',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 2,
            isMyMessage: false,
            name: 'Daniltrhb5',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'wwwwwwwww wwwwwwWWWWWWWW WWWWWWWWWWWWWWWWW WWWWWWWWWWWWW WWWWWWWWWWWWWWWWW WWWWAwwwwwwwww wwwwwwWWWWWWW WWWWWWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWA wwwww wwwwwwwwwWWWWWWWW WWWWWWWWWWWWW WWWWWWWWWWAwwwwwww wwwwwwwwWWW WWWWWWWWWWWWWW WWWWWWWWWWWWWW WWWWWWWWWWWWWWW WWWWWWWWWWWWWW Awwwwwwwwwwww wwwWWWWWWWWWW WWWWWWWWWWWWWW WWWWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWW WAwwwwwwwww wwwwwwWWWWWWWWWWW WWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWA',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 3,
            isMyMessage: true,
            name: 'Danildfg345t',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'тексregergт сообщкст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщения',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 4,
            isMyMessage: true,
            name: 'Dreg45anil',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'текст сообщения текст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщения',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 5,
            isMyMessage: false,
            name: 'D54ganil',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'текст сосообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщения',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 6,
            isMyMessage: true,
            name: 'Da54gnil',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'текст сообщения trhrtтекст соониятекст сообщениятекст сообщениятекст сообщениятекст сообщения',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 7,
            isMyMessage: true,
            name: 'D54anil',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'текст сообщения текст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщения',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 8,
            isMyMessage: true,
            name: 'Da5g4nil',
            time: '2020-01-02T14:22:22.000Z',
            message: 'текст сообщения текст сообщениятятекст сообщениятекст',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 9,
            isMyMessage: false,
            name: 'Da45gnil',
            time: '2020-01-02T14:22:22.000Z',
            message: 'текст',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
        ],
      },
      {
        id: 10,
        name: 'Masha',
        title: 'Error 100',
        time: '2020-01-02T14:22:22.000Z',
        content: 'this is message content',
        avatarURL:
          'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
        messages: [
          {
            id: 1,
            isMyMessage: true,
            name: 'Danilth',
            time: '2020-01-02T14:22:22.000Z',
            message:
              '',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 2,
            isMyMessage: false,
            name: 'Daniltrhb5',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'wwwwwwwww wwwwwwWWWWWWWW WWWWWWWWWWWWWWWWW WWWWWWWWWWWWW WWWWWWWWWWWWWWWWW WWWWAwwwwwwwww wwwwwwWWWWWWW WWWWWWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWA wwwww wwwwwwwwwWWWWWWWW WWWWWWWWWWWWW WWWWWWWWWWAwwwwwww wwwwwwwwWWW WWWWWWWWWWWWWW WWWWWWWWWWWWWW WWWWWWWWWWWWWWW WWWWWWWWWWWWWW Awwwwwwwwwwww wwwWWWWWWWWWW WWWWWWWWWWWWWW WWWWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWW WAwwwwwwwww wwwwwwWWWWWWWWWWW WWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWA',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 3,
            isMyMessage: true,
            name: 'Danildfg345t',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'тексregergт сообщкст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщения',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          // {
          //   id: 4,
          //   isMyMessage: true,
          //   name: 'Dreg45anil',
          //   time: '2020-01-02T14:22:22.000Z',
          //   message:
          //     'текст сообщения текст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщения',
          //   avatarURL:
          //     'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          // },
          // {
          //   id: 5,
          //   isMyMessage: false,
          //   name: 'D54ganil',
          //   time: '2020-01-02T14:22:22.000Z',
          //   message:
          //     'текст сосообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщения',
          //   avatarURL:
          //     'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          // },
          // {
          //   id: 6,
          //   isMyMessage: true,
          //   name: 'Da54gnil',
          //   time: '2020-01-02T14:22:22.000Z',
          //   message:
          //     'текст сообщения trhrtтекст соониятекст сообщениятекст сообщениятекст сообщениятекст сообщения',
          //   avatarURL:
          //     'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          // },
          // {
          //   id: 7,
          //   isMyMessage: true,
          //   name: 'D54anil',
          //   time: '2020-01-02T14:22:22.000Z',
          //   message:
          //     'текст сообщения текст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщения',
          //   avatarURL:
          //     'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          // },
          // {
          //   id: 8,
          //   isMyMessage: true,
          //   name: 'Da5g4nil',
          //   time: '2020-01-02T14:22:22.000Z',
          //   message: 'текст сообщения текст сообщениятятекст сообщениятекст',
          //   avatarURL:
          //     'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          // },
          // {
          //   id: 9,
          //   isMyMessage: false,
          //   name: 'Da45gnil',
          //   time: '2020-01-02T14:22:22.000Z',
          //   message: 'текст',
          //   avatarURL:
          //     'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          // },
        ],
      },
      {
        id: 11,
        name: 'Danil',
        title: ',eError 500',
        time: '2020-01-02T14:22:22.000Z',
        content: 'this is message content',
        avatarURL:
          'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
        messages: [
          {
            id: 1,
            isMyMessage: true,
            name: 'Danilth',
            time: '2020-01-02T14:22:22.000Z',
            message:
              '',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 2,
            isMyMessage: false,
            name: 'Daniltrhb5',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'wwwwwwwww wwwwwwWWWWWWWW WWWWWWWWWWWWWWWWW WWWWWWWWWWWWW WWWWWWWWWWWWWWWWW WWWWAwwwwwwwww wwwwwwWWWWWWW WWWWWWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWA wwwww wwwwwwwwwWWWWWWWW WWWWWWWWWWWWW WWWWWWWWWWAwwwwwww wwwwwwwwWWW WWWWWWWWWWWWWW WWWWWWWWWWWWWW WWWWWWWWWWWWWWW WWWWWWWWWWWWWW Awwwwwwwwwwww wwwWWWWWWWWWW WWWWWWWWWWWWWW WWWWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWW WAwwwwwwwww wwwwwwWWWWWWWWWWW WWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWA',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 3,
            isMyMessage: true,
            name: 'Danildfg345t',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'тексregergт сообщкст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщения',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 4,
            isMyMessage: true,
            name: 'Dreg45anil',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'текст сообщения текст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщения',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 5,
            isMyMessage: false,
            name: 'D54ganil',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'текст сосообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщения',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 6,
            isMyMessage: true,
            name: 'Da54gnil',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'текст сообщения trhrtтекст соониятекст сообщениятекст сообщениятекст сообщениятекст сообщения',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 7,
            isMyMessage: true,
            name: 'D54anil',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'текст сообщения текст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщения',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 8,
            isMyMessage: true,
            name: 'Da5g4nil',
            time: '2020-01-02T14:22:22.000Z',
            message: 'текст сообщения текст сообщениятятекст сообщениятекст',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 9,
            isMyMessage: false,
            name: 'Da45gnil',
            time: '2020-01-02T14:22:22.000Z',
            message: 'текст',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
        ],
      },
      {
        id: 12,
        name: 'Boris',
        title: 'Error 400',
        time: '2020-01-02T14:22:22.000Z',
        content: 'this is message content',
        avatarURL:
          'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
        messages: [
          {
            id: 1,
            isMyMessage: true,
            name: 'Danilth',
            time: '2020-01-02T14:22:22.000Z',
            message:
              '',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 2,
            isMyMessage: false,
            name: 'Daniltrhb5',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'wwwwwwwww wwwwwwWWWWWWWW WWWWWWWWWWWWWWWWW WWWWWWWWWWWWW WWWWWWWWWWWWWWWWW WWWWAwwwwwwwww wwwwwwWWWWWWW WWWWWWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWA wwwww wwwwwwwwwWWWWWWWW WWWWWWWWWWWWW WWWWWWWWWWAwwwwwww wwwwwwwwWWW WWWWWWWWWWWWWW WWWWWWWWWWWWWW WWWWWWWWWWWWWWW WWWWWWWWWWWWWW Awwwwwwwwwwww wwwWWWWWWWWWW WWWWWWWWWWWWWW WWWWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWW WAwwwwwwwww wwwwwwWWWWWWWWWWW WWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWA',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 3,
            isMyMessage: true,
            name: 'Danildfg345t',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'тексregergт сообщкст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщения',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 4,
            isMyMessage: true,
            name: 'Dreg45anil',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'текст сообщения текст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщения',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 5,
            isMyMessage: false,
            name: 'D54ganil',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'текст сосообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщения',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 6,
            isMyMessage: true,
            name: 'Da54gnil',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'текст сообщения trhrtтекст соониятекст сообщениятекст сообщениятекст сообщениятекст сообщения',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 7,
            isMyMessage: true,
            name: 'D54anil',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'текст сообщения текст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщения',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 8,
            isMyMessage: true,
            name: 'Da5g4nil',
            time: '2020-01-02T14:22:22.000Z',
            message: 'текст сообщения текст сообщениятятекст сообщениятекст',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 9,
            isMyMessage: false,
            name: 'Da45gnil',
            time: '2020-01-02T14:22:22.000Z',
            message: 'текст',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
        ],
      },
      {
        id: 13,
        name: 'Anton',
        title: 'Error 300',
        time: '2020-01-02T14:22:22.000Z',
        content: 'this is message content',
        avatarURL:
          'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
        messages: [
          {
            id: 1,
            isMyMessage: true,
            name: 'Danilth',
            time: '2020-01-02T14:22:22.000Z',
            message:
              '',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 2,
            isMyMessage: false,
            name: 'Daniltrhb5',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'wwwwwwwww wwwwwwWWWWWWWW WWWWWWWWWWWWWWWWW WWWWWWWWWWWWW WWWWWWWWWWWWWWWWW WWWWAwwwwwwwww wwwwwwWWWWWWW WWWWWWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWA wwwww wwwwwwwwwWWWWWWWW WWWWWWWWWWWWW WWWWWWWWWWAwwwwwww wwwwwwwwWWW WWWWWWWWWWWWWW WWWWWWWWWWWWWW WWWWWWWWWWWWWWW WWWWWWWWWWWWWW Awwwwwwwwwwww wwwWWWWWWWWWW WWWWWWWWWWWWWW WWWWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWW WAwwwwwwwww wwwwwwWWWWWWWWWWW WWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWA',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 3,
            isMyMessage: true,
            name: 'Danildfg345t',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'тексregergт сообщкст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщения',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 4,
            isMyMessage: true,
            name: 'Dreg45anil',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'текст сообщения текст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщения',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 5,
            isMyMessage: false,
            name: 'D54ganil',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'текст сосообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщения',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 6,
            isMyMessage: true,
            name: 'Da54gnil',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'текст сообщения trhrtтекст соониятекст сообщениятекст сообщениятекст сообщениятекст сообщения',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 7,
            isMyMessage: true,
            name: 'D54anil',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'текст сообщения текст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщения',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 8,
            isMyMessage: true,
            name: 'Da5g4nil',
            time: '2020-01-02T14:22:22.000Z',
            message: 'текст сообщения текст сообщениятятекст сообщениятекст',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 9,
            isMyMessage: false,
            name: 'Da45gnil',
            time: '2020-01-02T14:22:22.000Z',
            message: 'текст',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
        ],
      },
      {
        id: 14,
        name: 'Katya',
        title: 'FError 200',
        time: '2020-01-02T14:22:22.000Z',
        content: 'this is message content',
        avatarURL:
          'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
        messages: [
          {
            id: 1,
            isMyMessage: true,
            name: 'Danilth',
            time: '2020-01-02T14:22:22.000Z',
            message:
              '',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 2,
            isMyMessage: false,
            name: 'Daniltrhb5',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'wwwwwwwww wwwwwwWWWWWWWW WWWWWWWWWWWWWWWWW WWWWWWWWWWWWW WWWWWWWWWWWWWWWWW WWWWAwwwwwwwww wwwwwwWWWWWWW WWWWWWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWA wwwww wwwwwwwwwWWWWWWWW WWWWWWWWWWWWW WWWWWWWWWWAwwwwwww wwwwwwwwWWW WWWWWWWWWWWWWW WWWWWWWWWWWWWW WWWWWWWWWWWWWWW WWWWWWWWWWWWWW Awwwwwwwwwwww wwwWWWWWWWWWW WWWWWWWWWWWWWW WWWWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWW WAwwwwwwwww wwwwwwWWWWWWWWWWW WWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWA',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 3,
            isMyMessage: true,
            name: 'Danildfg345t',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'тексregergт сообщкст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщения',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 4,
            isMyMessage: true,
            name: 'Dreg45anil',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'текст сообщения текст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщения',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 5,
            isMyMessage: false,
            name: 'D54ganil',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'текст сосообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщения',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 6,
            isMyMessage: true,
            name: 'Da54gnil',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'текст сообщения trhrtтекст соониятекст сообщениятекст сообщениятекст сообщениятекст сообщения',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 7,
            isMyMessage: true,
            name: 'D54anil',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'текст сообщения текст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщения',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 8,
            isMyMessage: true,
            name: 'Da5g4nil',
            time: '2020-01-02T14:22:22.000Z',
            message: 'текст сообщения текст сообщениятятекст сообщениятекст',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 9,
            isMyMessage: false,
            name: 'Da45gnil',
            time: '2020-01-02T14:22:22.000Z',
            message: 'текст',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
        ],
      },
      {
        id: 15,
        name: 'Masha',
        title: 'Error 100',
        time: '2020-01-02T14:22:22.000Z',
        content: 'this is message content',
        avatarURL:
          'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
        messages: [
          {
            id: 1,
            isMyMessage: true,
            name: 'Danilth',
            time: '2020-01-02T14:22:22.000Z',
            message:
              '',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          // {
          //   id: 2,
          //   isMyMessage: false,
          //   name: 'Daniltrhb5',
          //   time: '2020-01-02T14:22:22.000Z',
          //   message:
          //     'wwwwwwwww wwwwwwWWWWWWWW WWWWWWWWWWWWWWWWW WWWWWWWWWWWWW WWWWWWWWWWWWWWWWW WWWWAwwwwwwwww wwwwwwWWWWWWW WWWWWWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWA wwwww wwwwwwwwwWWWWWWWW WWWWWWWWWWWWW WWWWWWWWWWAwwwwwww wwwwwwwwWWW WWWWWWWWWWWWWW WWWWWWWWWWWWWW WWWWWWWWWWWWWWW WWWWWWWWWWWWWW Awwwwwwwwwwww wwwWWWWWWWWWW WWWWWWWWWWWWWW WWWWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWW WAwwwwwwwww wwwwwwWWWWWWWWWWW WWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWA',
          //   avatarURL:
          //     'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          // },
          // {
          //   id: 3,
          //   isMyMessage: true,
          //   name: 'Danildfg345t',
          //   time: '2020-01-02T14:22:22.000Z',
          //   message:
          //     'тексregergт сообщкст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщения',
          //   avatarURL:
          //     'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          // },
          // {
          //   id: 4,
          //   isMyMessage: true,
          //   name: 'Dreg45anil',
          //   time: '2020-01-02T14:22:22.000Z',
          //   message:
          //     'текст сообщения текст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщения',
          //   avatarURL:
          //     'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          // },
          // {
          //   id: 5,
          //   isMyMessage: false,
          //   name: 'D54ganil',
          //   time: '2020-01-02T14:22:22.000Z',
          //   message:
          //     'текст сосообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщения',
          //   avatarURL:
          //     'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          // },
          // {
          //   id: 6,
          //   isMyMessage: true,
          //   name: 'Da54gnil',
          //   time: '2020-01-02T14:22:22.000Z',
          //   message:
          //     'текст сообщения trhrtтекст соониятекст сообщениятекст сообщениятекст сообщениятекст сообщения',
          //   avatarURL:
          //     'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          // },
          // {
          //   id: 7,
          //   isMyMessage: true,
          //   name: 'D54anil',
          //   time: '2020-01-02T14:22:22.000Z',
          //   message:
          //     'текст сообщения текст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщения',
          //   avatarURL:
          //     'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          // },
          // {
          //   id: 8,
          //   isMyMessage: true,
          //   name: 'Da5g4nil',
          //   time: '2020-01-02T14:22:22.000Z',
          //   message: 'текст сообщения текст сообщениятятекст сообщениятекст',
          //   avatarURL:
          //     'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          // },
          // {
          //   id: 9,
          //   isMyMessage: false,
          //   name: 'Da45gnil',
          //   time: '2020-01-02T14:22:22.000Z',
          //   message: 'текст',
          //   avatarURL:
          //     'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          // },
        ],
      },
      {
        id: 16,
        name: 'Anton',
        title: 'Error 300',
        time: '2020-01-02T14:22:22.000Z',
        content: 'this is message content',
        avatarURL:
          'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
        messages: [
          {
            id: 1,
            isMyMessage: true,
            name: 'Danilth',
            time: '2020-01-02T14:22:22.000Z',
            message:
              '',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 2,
            isMyMessage: false,
            name: 'Daniltrhb5',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'wwwwwwwww wwwwwwWWWWWWWW WWWWWWWWWWWWWWWWW WWWWWWWWWWWWW WWWWWWWWWWWWWWWWW WWWWAwwwwwwwww wwwwwwWWWWWWW WWWWWWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWA wwwww wwwwwwwwwWWWWWWWW WWWWWWWWWWWWW WWWWWWWWWWAwwwwwww wwwwwwwwWWW WWWWWWWWWWWWWW WWWWWWWWWWWWWW WWWWWWWWWWWWWWW WWWWWWWWWWWWWW Awwwwwwwwwwww wwwWWWWWWWWWW WWWWWWWWWWWWWW WWWWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWW WAwwwwwwwww wwwwwwWWWWWWWWWWW WWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWWW WWWWWWWWWWWWWWWA',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 3,
            isMyMessage: true,
            name: 'Danildfg345t',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'тексregergт сообщкст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщения',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 4,
            isMyMessage: true,
            name: 'Dreg45anil',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'текст сообщения текст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщения',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 5,
            isMyMessage: false,
            name: 'D54ganil',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'текст сосообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщения',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 6,
            isMyMessage: true,
            name: 'Da54gnil',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'текст сообщения trhrtтекст соониятекст сообщениятекст сообщениятекст сообщениятекст сообщения',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 7,
            isMyMessage: true,
            name: 'D54anil',
            time: '2020-01-02T14:22:22.000Z',
            message:
              'текст сообщения текст сообщениятекст сообщениятекст сообщениятекст сообщениятекст сообщения',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 8,
            isMyMessage: true,
            name: 'Da5g4nil',
            time: '2020-01-02T14:22:22.000Z',
            message: 'текст сообщения текст сообщениятятекст сообщениятекст',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
          {
            id: 9,
            isMyMessage: false,
            name: 'Da45gnil',
            time: '2020-01-02T14:22:22.000Z',
            message: 'текст',
            avatarURL:
              'https://pixelbox.ru/wp-content/uploads/2020/12/ava-vk-cats-76.jpg',
          },
        ],
      },
    ],
  
};
